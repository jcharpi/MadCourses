...
def import_csv_to_database(csv_path: str, db_path: Optional[str] = None):
    if db_path is None:
        db_path = os.path.join(os.path.dirname(__file__), "courses.db")
...
def generate_and_store_embeddings(db_path: Optional[str] = None, model_name: str = "all-MiniLM-L12-v2"):
    if db_path is None:
        db_path = os.path.join(os.path.dirname(__file__), "courses.db")
...
def load_embeddings_from_database(db_path: Optional[str] = None) -> Tuple[List[Dict[str, Any]], np.ndarray]:
    if db_path is None:
        db_path = os.path.join(os.path.dirname(__file__), "courses.db")
...
def get_database_stats(db_path: Optional[str] = None):
    if db_path is None:
        db_path = os.path.join(os.path.dirname(__file__), "courses.db")
